CREATE TABLE IF NOT EXISTS `mit`.`address` (
...
CREATE TABLE IF NOT EXISTS `mit`.`users` (
    ...
    `updatedate` DATETIME NULL,
    PRIMARY KEY (`id`),
    FOREIGN KEY (`addressid`) REFERENCES `mit`.`address` (`addressid`)
    )
    ENGINE = InnoDB;

INSERT INTO `mit`.`address` VALUES (1, 1000001 , "東京都新宿区西新宿１−１−１", now(), now());
